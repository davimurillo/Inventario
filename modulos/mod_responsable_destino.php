 ...
 <!-- Head app -->
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Configuración
        <small>Responsable Destino</small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Responsable Destino</li>
      </ol>
    </section>
...
##  *** put a primary key on the first place 
	...
	$sql="SELECT id_tienda, tx_marca, (tx_descripcion) AS tx_descripcion2, tx_descripcion, tx_marca, tx_direccion, telefono_tienda, tx_url, correo_tienda, ('<i class=\"fa fa-info\"></i>') as icono_info FROM vie_tbl_tiendas_destino ";
	
	if (isset($_GET['buscar_clientes'])){
		$sql.=" WHERE upper(CONCAT(tx_marca || ' ' || tx_descripcion)) LIKE '%".strtoupper($_GET['buscar_clientes'])."%' or  upper(tx_marca) LIKE '%".strtoupper($_GET['buscar_clientes'])."%' OR upper(tx_descripcion) LIKE '%".strtoupper($_GET['buscar_clientes'])."%' OR upper(tx_direccion) LIKE '%".strtoupper($_GET['buscar_clientes'])."%' OR upper(telefono_tienda) LIKE '%".strtoupper($_GET['buscar_clientes'])."%'  OR upper(correo_tienda) LIKE '%".strtoupper($_GET['buscar_clientes'])."%' ";
	}
