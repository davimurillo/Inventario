            ...
            <!-- form start -->
              ...
              <div class="box-body">
				...
				<div class="form-group">
					...
					<div id="tipo_motivo" class="col-xs-6">
						<label>Nº DE MOTIVO</label>
						<div class="input-group date">
							<div class="input-group-addon">
								<i class="fa fa-arrow-circle-right"></i>
							</div>
							<input id="n_motivo" type="text" class="form-control pull-right" placeholder="N° de Motivo" required="required">
						</div>
					</div>
					...
					<div id="colaborador" class="col-xs-6">
						<label>Tienda/Colaborador</label>
						<select id="responsable_destino" class="form-control select2"  required="required">
							<option value="">Seleccione...</option>
  ...
  $(function () {
   ...
   //Date picker
    ...
    $( "#terminar" ).click(function() {
		...
		responsable_destino=$('#responsable').val();
		origen=$('#origen').val();
		fe_entrada=$('#fe_entrada').val();
		fe_hasta=$('#fe_hasta').val();
		ticket=$('#ticket').val();
		guia=$('#guia').val();
		n_motivo=$('#n_motivo').val();
		reporte=$("#reporte option:selected").text();
		tipo=$('#tipo_producto').val();
		url="reportes/"+$('#reporte').val()+"?un="+un+"&motivo="+motivo+"&proveedor="+proveedor+"&origen="+origen+"&fe_entrada="+fe_entrada+"&fe_hasta="+fe_hasta+"&ticket="+ticket+"&guia="+guia+"&tipo="+tipo+"&reporte="+reporte+"&responsable_destino="+responsable_destino+"&nmotivo="+n_motivo;
	...
	function cambiar_ubicacion(unidad_negocio){
			...
			$('#colaborador').load ('mod_eventos.php', { unidad: unidad_negocio, evento:32 });
	}
