...
<h3>STOCK DE INVENTARIO - EQUIPOS DE BAJA - LEASING</h3><hr>
...
<table class="mitabla table table-bordered table-striped" style="font-size:12px; color:#000; width:100%" >
  ...
  <?php 
    ...
    tx_marca,
    tx_modelo,
    (SELECT tx_nombre_tipo FROM cfg_tipo_producto WHERE a.id_tipo_producto=id_tipo_producto) as tx_nombre_tipo  FROM mod_producto a WHERE estatus=7 AND ultimo_movimiento=1 AND id_tienda=1 AND id_condicion=80 AND id_tipo_motivo=28";
  
	if (isset($_GET['tipo']) && $_GET['tipo']!='') $sql.=" AND id_tipo_producto=".$_GET['tipo']."";
	
	$sql.=" ORDER BY tx_nombre_tipo";
