...
<table class="mitabla table table-bordered table-striped" style="font-size:12px; color:#000; width:100%" >
  ...
  <?php 
	...
	(SELECT  CONCAT(d.tx_descripcion , ' ' , d.tx_apellido_paterno , ' ' , d.tx_apellido_materno , ' ' , d.tx_puesto , ' ' , d.tx_dni) FROM mod_movimiento c, mod_responsable_destino d WHERE c.id_responsable_destino=d.id_tienda and 
c.id_producto=a.id_producto ORDER BY c.fe_movimiento DESC, c.fe_actualizada DESC LIMIT 1) as descripcion_colaborador,
	(SELECT CONCAT(tx_descripcion , ' ' , tx_apellido_paterno , ' ' , tx_apellido_materno , ' ' , tx_puesto , ' ' , tx_dni) FROM mod_tienda WHERE a.id_tienda=id_tienda) as descripcion,
    n_stock,
    tx_serial,
    tx_placati,
    tx_ngr,
    ...
    tx_modelo,
    (SELECT tx_nombre_tipo FROM cfg_tipo_producto WHERE id_tipo_producto=a.id_tipo_producto) as tx_tipo_producto  
	FROM mod_producto a ";
  ...
  <tr>
    ...
    <td style="text-align:left" ><?php echo $row['descripcion']."<br>".$row['descripcion_colaborador']; ?> </td>
