...
tx_accesorios,
tx_observacion
FROM vie_tbl_movimiento a WHERE id_tipo_movimiento=2 and (SELECT id_estatus_movimiento FROM mod_movimiento WHERE id_movimiento=a.id_movimiento) =1 ";
...
if ($_GET['origen']>0){
  ...
  while ($row=dregistro($res)){
	if ($tipo_motivo!=$row['tipo_motivo']) { 
		...
		$c=0;
	?>
	<tr> <td colspan="14" style="text-align:center; background-color:#ccc; "><?php echo $row['tipo_motivo']; ?></td> </tr>
	<?php $c+=1;  ?>
	<tr>
